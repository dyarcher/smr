...
// work variables
double B0 = 3.0;
double sysGyro = 42.5763847446*6.28318530718e6;
double forwardT = 0;
double precTauRF = 1e-6;
double precTauGrad = 1e-3;
double precTauDelay = 1e-3;
double precTauADC = 1e-3;
double precTauActive = 1e-3;
bool precTausSet = false;
std::vector<double> smrT1s;
std::vector<double> smrT2s;
std::vector<double> smrGyros;
std::vector<double> smrCSs;
std::vector<double> smrGyrosInternal;
std::vector<double> smrCSsInternal;
...
bool smr_init() {
	if (!smrinit) {
		...
		std::cout << "- Primary magnetic field (B0): " << B0 << " T" << std::endl;
		std::cout << "- Primary gyromagnetic ratio (gamma): " << std::setprecision(16) << sysGyro/6.28318530718e6 << " MHz/T" << std::endl;
		smrinit = true;
	}
	return true;
}
...
// set global MR parameters
void smrsetMRsystem(char* line2) {
	std::cout << "Setting MR system parameters:" << std::endl;
	// parse command line
	const char s[2] = " ";
	char *token;
	token = strtok(line2, s);
	int i = 0;
	while (token != NULL) {
		if (i == 0) { // primary magnetic field
			B0 = std::stod(token);
			std::cout << "- Primary magnetic field: " << B0 << std::endl;
		} else if (i == 1) {
			sysGyro = std::stod(token)*6.28318530718e6;
			std::cout << "- Gyromagnetic ratio: " << sysGyro << std::endl;
		} else if (i == 2) {
			sysGyro += std::stod(token)*6.28318530718/B0;
		} else if (i == 3) {
			if (!precTausSet) precTauRF = std::stod(token);
			std::cout << "- RF time step: " << precTauRF << std::endl;
		} else if (i == 4) {
			if (!precTausSet) precTauGrad = std::stod(token);
			std::cout << "- Gradient time step: " << precTauGrad << std::endl;
		} else if (i == 5) {
			if (!precTausSet) precTauDelay = std::stod(token);
			std::cout << "- Delay time step: " << precTauDelay << std::endl;
		} else if (i == 6) {
			if (!precTausSet) precTauADC = std::stod(token);
			std::cout << "- ADC time step: " << precTauADC << std::endl;
		}
		token = strtok(NULL, s);
		i += 1;
	}
...
void smrsetMR(char* line2) {
	...
	while (token != NULL) {
		if (i == 0) { // gyromagnetic ratio
			...
			smrGyrosInternal.push_back(std::stod(token)*6.28318530718e6);
		} else if (i == 1) { // T1
			smrT1s.push_back(std::stod(token));
		} else if (i == 2) { // T2
			smrT2s.push_back(std::stod(token));
		} else if (i == 3) { // chemical shift
			smrCSs.push_back(std::stod(token)*6.28318530718/B0);
			smrCSsInternal.push_back(std::stod(token)*6.28318530718/B0);
		}
		token = strtok(NULL, s);
		i += 1;
	}
...
void smr_relax(double tau, double T1, double T2, double HPT1, double M0, double &mx, double &my, double &mz, double relaxing_time)
{
	mx *= exp(-tau/T2);
	my *= exp(-tau/T2);
	double currrelaxtime = log(1-mz)*T1*-1 + tau;
	if (abs(mz) < 1) mz = (1-exp(-currrelaxtime/T1));
	if (HPT1 > 1e-12){
...
// smrsetGyro: sets gyromagnetic ratio of each particle
double smrsetGyro(int spinspecies) {
	if (smrGyros.size() >= spinspecies) return smrGyros[spinspecies-1];
	return -1;
}
// smrsetCS: sets chemical shift of each particle
double smrsetCS(int spinspecies) {
	if (smrCSs.size() >= spinspecies) return smrCSs[spinspecies-1];
	return -1;
}
...
double mag_at_time_dummy(double t, std::vector<double> &ch_time, std::vector<double> &ch_amp, double pos, int &curr_time_idx)
{
	double B = 0;
	if (ch_time.size() > 0) {
		// get magnetization values based on time/index
...
void smr_timestep(double t, double smolTau) {
	smr_init();
	double tau = smolTau*1;
	double start_t = t*1;
	double temp_t = t*1;
	// check if need to go back in time for pulse
	double rfa_val = mag_at_time_dummy(temp_t, rf_time, rf_amp, 1, curr_time_idx_rf);
	if (abs(rfa_val) > 0) {
		tau = precTauRF*1;
		if (precTauActive > precTauRF) temp_t = t - smolTau + precTauRF;
		precTauActive = precTauRF*1;
	} else {
		tau = smolTau*1;
		precTauActive = smolTau*1;
	}
	double last_adc = temp_t-smolTau;
	while (temp_t < start_t+smolTau) {
		double adc_ampa = adc_at_time(temp_t, adc_time, adc_amp, curr_time_idx_adc);
		ro_mx = 0;
		ro_my = 0;
		ro_mz = 0;
		double rfa = mag_at_time(temp_t, rf_time, rf_amp, 1, curr_time_idx_rf);
		double rfn = mag_at_time(temp_t, rfneg_time, rfneg_amp, 1, curr_time_idx_rfn);
		if (rfn > 0) {
			rfa *= -1;
		}
		double rfp = phase_at_time(temp_t, rfp_time, rfp_amp, 1, curr_time_idx_rfp);
		std::vector<double> Beff = { rfa*sin(rfp), rfa*cos(rfp), 0 }; // adjusted for phase cycling
		std::vector<double> Grads = { 0, 0, 0 };
		Grads[0] = mag_at_time_global(temp_t, gx_time, gx_amp, curr_time_idx_gx);
		Grads[1] = mag_at_time_global(temp_t, gy_time, gy_amp, curr_time_idx_gy);
		Grads[2] = mag_at_time_global(temp_t, gz_time, gz_amp, curr_time_idx_gz);
		// set timestep based on MR system state
		//if (abs(rfa) > 0 | rfa_check) tau = precTauRF;
		//else if (abs(Grads[0])>0 | abs(Grads[1])>0 | abs(Grads[2])>0) tau = precTauGrad;
		//else tau = precTauDelay;
		//if (temp_t+tau > start_t+smolTau) tau = tau*1+temp_t-start_t-smolTau;
		...
		for (int i = 0; i < smrblocks.size(); i++) {
			...
			if (smrblocks[i].gyromag == -1) smrblocks[i].Mz = smrBasePols[smrblocks[i].spinspecies-1];
			if (smrblocks[i].gyromag == -1) smrblocks[i].gyromag = smrGyros[smrblocks[i].spinspecies-1];
			if (smrblocks[i].CS == -1) smrblocks[i].CS = smrCSs[smrblocks[i].spinspecies-1];
			double gyromag = smrblocks[i].gyromag + smrblocks[i].CS;
			...
			double Boffres = (gyromag - sysGyro) / (sysGyro) * B0; // correct for resonance deviation from rotating frame
		...
		if (adc_timings.size() > 0) {
			if (abs(temp_t-adc_timings[idx_adc]) < samplingtime/2) {
				...
				std::vector<double> ro = { temp_t, ro_mx, ro_my, ro_mz }; // phase cycling
				readout_values.push_back(ro);
				idx_adc += 1;
			}
		} else if (adc_ampa > 0 & last_adc + precTauADC <= temp_t) {
			rotquat_hardangle(ro_rfp, ro_mx, ro_my, ro_mz); // phase cycling
			std::vector<double> ro = { temp_t, ro_mx, ro_my, ro_mz }; // phase cycling
			readout_values.push_back(ro);
			stored_rfp = rfp;
		...
		if (smrdebug_new) { 
			if (debug_idx >= debugsamplefactor) {
				debugfile << std::setprecision(16) << temp_t << " ";
				debugfile << rfa << " ";
				debugfile << rfn << " ";
				debugfile << rfp << " ";
				debugfile << Grads[0] << " ";
				...
				debugfile << ro_my << " ";
				debugfile << ro_mz << " ";
				debugfile << sysGyro/6.28318530718e6 << " ";
				debugfile << tau << "\n";
				debug_idx = 0;
			}
			debug_idx += 1;
		...
		if (calckspace) {
			kx += mag_at_time(temp_t, gx_time, gx_amp, 1, curr_time_idx_gx) + rfa*sin(rfp);
			ky += mag_at_time(temp_t, gy_time, gy_amp, 1, curr_time_idx_gy) + rfa*cos(rfp);
			kz += mag_at_time(temp_t, gz_time, gz_amp, 1, curr_time_idx_gz);
