...
enum CMDcode cmdsmrsetMRsystem(simptr sim,cmdptr cmd,char *line2);
...
//#ifdef __smr_h__
	...
	else if(!strcmp(word,"smrsetMRsystem")) return cmdsmrsetMRsystem(sim,cmd,line2);
...
enum CMDcode cmdsmrtotmag(simptr sim,cmdptr cmd,char *line2) {   // track total magnetization
	...
	scmdfprintf(cmd->cmds,fptr,",");
	scmdfprintf(cmd->cmds,fptr,"%lf",ymag);       // print My to file
	scmdfprintf(cmd->cmds,fptr,",");
	scmdfprintf(cmd->cmds,fptr,"%lf",zmag);       // print Mz to file
	scmdfprintf(cmd->cmds,fptr,"\n");                             // print newline to file
	scmdflush(fptr);
	return CMDok;
}
...
enum CMDcode cmdsmrmolstate(simptr sim,cmdptr cmd,char *line2) {   // save molecule state to file using associated smr block
	...
	scmdfprintf(cmd->cmds,fptr,"%g",sim->time);                   // print sim time to file
	scmdfprintf(cmd->cmds,fptr," ");                              // print space to file
	scmdfprintf(cmd->cmds,fptr,"%g%,%s%,%s",sim->time,sim->mols->spname[mptr->ident],molms2string(mptr->mstate,string));       // print molecule state to file
	scmdfprintf(cmd->cmds,fptr,"\n");                             // print newline to file
	scmdflush(fptr);
	return CMDok;
}
...
enum CMDcode cmdsmrsetMRsystem(simptr sim,cmdptr cmd,char *line2) {   // set gyromagnetic ratio, chemical shifts, T1, T2 for multiple peaks
	int er,dataid;
	char string[STRCHAR];
	if(line2 && !strcmp(line2 ,"cmdtype")) return CMDobserve;     // check command is valid
	smrsetMRsystem(line2);
	return CMDok;
}
