...
// BEMATHMR is a magnetic resonance model compatible with the Smoldyn biochemical simulation platform.
// bemathmr.cpp contains core code for BEMATHMR. Shared global variables created here are defined as externs in the Smoldyn header file.
...
bool bemathmrinit = false;
char *seqfilename; // name of sequence file
char *seqtypetok; // "discrete" for scanner discrete timecourse, "pulseq" for pulseq format; set by bemathmrparseq command
std::string seqtype; // "discrete" for scanner discrete timecourse, "pulseq" for pulseq format; set by bemathmrparseq command
...
bool bemathmrdebug = false;
bool bemathmrdebug_new = false;
...
// work variables
std::vector<double> bemathmrT1s;
std::vector<double> bemathmrT2s;
std::vector<double> bemathmrGyros;
std::vector<double> bemathmrCSs;
std::vector<double> bemathmrHPT1s;
std::vector<double> bemathmrBasePols;
std::vector<bemathmrblockptr> bemathmrblocks;
double ro_mx = 0.;
double ro_my = 0.;
double ro_mz = 0.;
double ro_rfp = 0.;
double stored_rfp = 0.;
double relaxingtime = 0.;
...
bool bemathmr_init() {
	if (!bemathmrinit) {
		std::cout << std::endl << "Smoldyn MR Module (BEMATHMR) Initialized" << std::endl;
		bemathmrinit = true;
	}
	return true;
}

// set quality control/monitoring/debugging paramters
void bemathmrinfo(char* line2) {
	bemathmr_init();
	// parse command line
	const char s[2] = " ";
	char *token;
...
// bemathmr_relax: relax magnetization
void bemathmr_relax(double tau, double T1, double T2, double HPT1, double M0, double &mx, double &my, double &mz)
{
	mx *= exp(-tau/T2);
	my *= exp(-tau/T2);
	mz *= exp(-tau/T1);
	mz += M0*(1-exp(-tau/T1));
	if (HPT1 > 1e-12) mz *= exp(-tau/HPT1);
}

// get_T1: get T1 value from current position
double get_T1(double posx, double posy, double posz, int spinspecies) {
	return bemathmrT1s[spinspecies-1];
}
// get_T2: get T2 value from current position
double get_T2(double posx, double posy, double posz, int spinspecies) {
	return bemathmrT2s[spinspecies-1];
}
// get_HPT1: get hyperpolarized T1 value from species
double get_HPT1(int spinspecies) {
	return bemathmrHPT1s[spinspecies-1];
}
// get_BasePol: get base polarization (M0) value from species
double get_BasePol(int spinspecies) {
	return bemathmrBasePols[spinspecies-1];
}
// get_gyro: get effective gyromagnetic ratio (calculated with chemical shift) for species
double get_effective_gyromag(int spinspecies) {
	return bemathmrGyros[spinspecies-1] + 0;//bemathmrCSs[spinspecies-1];
}
...
void read_adc(char *filename)
{
	...
	//adc_timings.resize(0); // clear ADC timings from pulse file, if any were provided; explicit bemathmrreadadc command takes precedence
...
{
	std::cout << "- BEMATHMR: Writing readout line " << lines_read << " (length " << readout_values.size() << ") to " << filename << "." << std::endl;
	std::ofstream outfile;
		...
		for (auto it = ch_time.begin() + curr_time_idx; it != ch_time.end(); it++) {
			...
			if (ch_time[idx] >= t) {
				...
				if (seqtype.compare("discrete") == 0) { // discrete sequence amplitudes are exact
					...
					B += ch_amp[idx] / (bemathmrGyros[0]/6.28318530718) * pos;
				}
				break;
			}
		}
		...
		for (auto it = ch_time.begin() + curr_time_idx; it != ch_time.end(); it++) {
			...
			if (ch_time[idx] >= t) {
				...
				if (seqtype.compare("discrete") == 0) { // discrete sequence amplitudes are exact
					...
					B += ch_amp[idx] / (bemathmrGyros[0]/6.28318530718);
				}
				break;
			}
		}
...
// setparams
void bemathmrparseparams(char *filename)
{
	std::cout << filename << std::endl;
}

// timestep function called in Smoldyn
void bemathmr_timestep(double t, double tau) {
	bemathmr_init();
	...
	for (int i = 0; i < bemathmrBasePols.size(); i++) {
		bemathmrBasePols[i] *= exp(-tau/bemathmrHPT1s[i]);
		if (bemathmrBasePols[i] < 0) bemathmrBasePols[i] = 0;
	}
	...
	for (int i = 0; i < bemathmrblocks.size(); i++) {
		bemathmrblocks[i].M0 = get_BasePol(bemathmrblocks[i].spinspecies);
		if (bemathmrblocks[i].Mz > bemathmrblocks[i].M0) bemathmrblocks[i].Mz = bemathmrblocks[i].M0;
		double gyromag = bemathmrGyros[bemathmrblocks[i].spinspecies-1] + bemathmrCSs[bemathmrblocks[i].spinspecies-1];//get_effective_gyromag(bemathmrblocks[i].spinspecies);
		if (gyromag < 1e-3) continue;
		double posx = bemathmrblocks[i].smolmptr->pos[0];
		double posy = bemathmrblocks[i].smolmptr->pos[1];
		double posz = bemathmrblocks[i].smolmptr->pos[2];
		...
		if (abs(Beff[0]) > 0 | abs(Beff[1]) > 0 | abs(Beff[2]) > 0) { // precession in the rotating frame
			//relaxingtime = 0.;
			rotquat(gyromag, tau, bemathmrblocks[i].Mx, bemathmrblocks[i].My, bemathmrblocks[i].Mz, Beff[0], Beff[1], Beff[2]);
		}
		if (abs(rfa) < 1e-12) { // relaxation when there's no excitation
			//relaxingtime += tau;
			bemathmrblocks[i].T1 = get_T1(posx, posy, posz, bemathmrblocks[i].spinspecies);
			bemathmrblocks[i].T2 = get_T2(posx, posy, posz, bemathmrblocks[i].spinspecies);
			bemathmr_relax(tau, bemathmrblocks[i].T1, bemathmrblocks[i].T2, bemathmrHPT1s[bemathmrblocks[i].spinspecies-1], bemathmrblocks[i].M0, bemathmrblocks[i].Mx, bemathmrblocks[i].My, bemathmrblocks[i].Mz);
		}
		ro_mx += bemathmrblocks[i].Mx;
		ro_my += bemathmrblocks[i].My;
		ro_mz += bemathmrblocks[i].Mz;
	}
	// readout
	if (adc_timings.size() > 0) {
		if (abs(t-adc_timings[idx_adc]) < samplingtime/2) {
			...
			//std::vector<double> ro = { t, ro_mx, ro_my, ro_mz }; // no phase cycling
